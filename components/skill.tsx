  ...
  return (
    <section id="skills" className="py-24 px-4 md:px-8 relative">
      ...
      >
        ...
        <TabsList className="bg-transparent border h-auto px-8 flex gap-8 mb-14 shadow-xs">
          {skills.map((category) => (
            <TabsTrigger
              key={category.id}
              value={category.id}
              className="relative bg-transparent border-none shadow-none px-0 py-2 rounded-none text-muted-foreground data-[state=active]:text-primary data-[state=active]:bg-transparent font-semibold text-base md:text-xl transition-all after:absolute after:bottom-0 after:left-0 after:h-0.5 after:w-full after:bg-primary after:scale-x-0 data-[state=active]:after:scale-x-100 after:transition-transform after:duration-300"
            >
              {category.label}
              {/* Just show first word for clean look */}
            </TabsTrigger>
          ))}
        </TabsList>
          ...
          {skills.map((category) => (
            ...
            >
              ...
              <div className="flex flex-col items-center">
                <div className="h-px w-15 bg-primary/30 mb-4" />
